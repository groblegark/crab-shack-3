...
function afterErrand(c, chain) {
  ...
  if (chain && !c.p.sick && (c.chainN || 0) < 2) {
    const e = pickErrand(c);
    const sameStop = e && (e.soup || e.tap != null ? false : e.biz === c.errandBiz);
    if (e && !e.selfCook && !sameStop && (e.soup || e.tap != null || bizOpenNow(e.biz))
        && errandDetour(c, e) <= CHAIN_PX) {
      c.chainN = (c.chainN || 0) + 1; c.errandCd = 0;
      if (e.soup || e.tap != null) startTapStop(c, e); else startErrand(c, e);
      return;
    }
  }
  c.chainN = 0;
