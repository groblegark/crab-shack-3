...
function updateSchedule(c, dt) {
  ...
  if (c.dayState === "home" && shackOpen() && tmin < 19.5 * 60 && c.errandCd <= 0 && errandWindow) {
...
// ---------------------------------------------------------------- errands
function bizStaffed(b) { return bizUnlocked(b) && allCrabs().some(k => k.duty && k.workBiz === b); }
...
function updateKitchen(c, dt) {
  ...
  if (c.kstate === "idle") {
    ...
    if (ownerFunds(bizKey) >= INGREDIENT_COST[c.cust.recipe.raw]) { c.kstate = "work"; c.workMax = c.workT = 0.6; c.slotKind = null; c.slot = -1; }
    ...
    if (c.workT <= 0) {
      if (c.stepIdx === -1) {
        ...
        debitBiz(bizKey, INGREDIENT_COST[c.carrying], c.x, FLOOR_Y - 40);
      }
...
function updateCustomers(dt) {
  const qi = { shack: 0, cleaners: 0, arcade: 0, showers: 0 };
...
cv.addEventListener("click", (ev) => {
  ...
  if (p.y >= PANEL_Y) {
    ...
    for (const key of Object.keys(BIZ)) {
      if (!bizUnlocked(key) || bizOwner(key) !== "player") continue;
