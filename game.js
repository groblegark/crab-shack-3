...
const BUS_SECS = 1.5;   // clearing a table is a swipe of the plates, not a shower-stall scrub
...
function drawCrab(c) {
  ...
  const working = (c.kstate === "work" || c.kstate === "cleaningStall" || c.kstate === "busingTable") && c.dayState === "working";
...
function manageRects() {
  ...
  const R = {
    ...
    // ---- SCHEDULE tab / PAY row group (its own band between the shop chips
    // and the roster rows; the roster's geometry below is deliberately
    // untouched). The track is a thumb-sized tap target: 12px tall, 120 wide,
    // and a tap anywhere along it sets the share.
    tipTrack: { x: x + 46, y: y + 46, w: 120, h: 12 },
    rows: [], cells: [],
    // ---- TOWN tab
