...
scenario("days off: wages skip exactly and the bill dips to match", () => {
  ...
  if (owed !== 22) return "BILL shows $" + owed + " wages with one of two crew off, expected $22";
  sim.runUntil("lastRentDay === day", { maxSteps: 30000 });
  if (sim.G("report.wages") !== 22) return "settlement paid $" + sim.G("report.wages") + " wages, expected the billed $22";
  ...
  if (pw !== 50 + 22 - 10) return "working PINCHY wallet " + pw + ", expected 62 (wage - house rent)";
